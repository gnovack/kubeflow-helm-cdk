...
export class EksStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props?: EksStackProps) {
    ...
    const kubernetesVersion =
      props?.kubernetesVersion ?? this.DEFAULT_KUBERNETES_VERSION;
    const albControllerVersion =
      props?.albControllerVersion ?? this.DEFAULT_ALB_CONTROLLER_VERSION;

    // Manually created secret
    const userPasswordSecret = Secret.fromSecretCompleteArn(
      this,
      "UserPasswordSecret",
      Env.SAMPLE_USER_PASSWORD_ARN
    );

    const kubernetesApiAccessPolicy = new PolicyStatement({
      actions: ["eks:AccessKubernetesApi", "eks:DescribeCluster"],
      resources: [`arn:aws:eks:*:${accountId}:cluster/*`],
    });
    ...
    const certManagerDeployment = new CertManagerDeployment(
      this,
    ...
    const istioIngress = new IstioIngressDeployment(
      this,
      "IstioIngressDeployment",
      {
        cluster: cluster,
    ...
    const secretsProvider = new SecretsProviderDeployment(
      this,
      "SecretsProviderDeployment",
      {
        cluster: cluster,
      }
    );
    ...
    const oidcAuthservice = new AuthServiceDeployment(
      this,
      "AuthServiceDeployment",
      {
        cluster: cluster,
